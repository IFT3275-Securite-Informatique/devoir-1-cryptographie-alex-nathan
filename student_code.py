...
# Obtenir la liste des tri-caractères les plus communs triée par ordre fréquence
def find_trigram_frequency():
    french_txt = generate_corpus()[:]
...
# Déchiffrer un texte avec la clé K
def decrypt_with_mapping(chunks, K):
    return ''.join([K[chunk] for chunk in chunks])
...
def decrypt(C):
    ...
    M = decrypt_with_mapping(chunks, mapping)
    compare_K(K, mapping)
